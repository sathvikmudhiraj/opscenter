...
import { useNotificationMetrics } from "./NotificationMetricsProvider";
...
export function NotificationList() {
  ...
  const { unreadCount, refreshUnreadCount } = useNotificationMetrics();

  async function load() {
    setLoading(true);
    try {
      // Add delay to prevent 429 errors from concurrent requests during initial load
      const delay = (ms: number) => new Promise(resolve => setTimeout(resolve, ms));
      await delay(400); // 400ms delay to stagger with other initial requests
      ...
      <div className="divide-y divide-slate-100">
        {items.map((item) => (
          <div key={item.id} className="flex flex-col gap-3 p-5 md:flex-row md:items-start md:justify-between">
            ...
            {!item.readAt ? <button type="button" onClick={() => markRead(item.id)} className="rounded-md border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50">Mark read</button> : <span className="text-xs font-semibold text-green-700">Read</span>}          </div>
        ))}
      </div>
