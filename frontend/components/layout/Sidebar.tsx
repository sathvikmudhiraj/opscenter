...
import { BarChart3, Bell, Boxes, ClipboardList, FileBarChart, FileClock, Gauge, HardHat, History, LogOut, Settings, ShieldCheck, UserCog, Users } from "lucide-react";
...
export function Sidebar({ role }: { role: UserRole }) {
  ...
  return (
    <aside className="flex min-h-screen w-72 flex-col bg-slate-950 px-4 py-5 text-slate-100">
      ...
      <button
        className="mt-auto flex items-center gap-3 rounded-md px-3 py-3 text-sm font-medium text-slate-300 transition hover:bg-white/10 hover:text-white"
        onClick={() => {
          clearSession();
